...
export type RenderMathFunc = (code: string, isDisplay: boolean) => string;
...
export class Renderer implements IRenderer {
  ...
  protected renderMath?: RenderMathFunc;
  ...
  public constructor(opts?: RenderOptions) {
    if (opts) {
      ...
      this.renderMath = opts.renderMath;
  ...
  renderMathBlock(ctx: RenderContext, mathBlockEl: MathBlock): void {
    ...
    const content = (
      this.enableLaTeX ? ctx.latexParser.tex(ctx.texCtx, new StringStream(mathBlockEl.content)) :
        mathBlockEl.content);
    if (this.renderMath) {
      ctx.html += this.renderMath(content, !mathBlockEl.inline);
    } else {
      ctx.html += '<script type="math/tex' + (mathBlockEl.inline ? '' : '; mode=display') + '">' + content + '</script>';
    }
  }
